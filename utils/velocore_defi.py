...
class Velocore(Approve, TgBot):
    ...
    def __init__(self, private_key, web3, number, log):
        ...
        self.number = number
        self.log = log
        self.address = Web3.to_checksum_address('0xd999E16e68476bC749A28FC14a0c3b6d7073F50c')
        self.abi = js.load(open('./abi/velocore.txt'))
    ...
    def buy_token(self, token_to_buy, value_eth, retry=0):
        self.log.info('Buy token Velocore')
        try:
            ...
            if value > balance:
                ...
                if value < Web3.to_wei(0.000001, 'ether'):
                    ...
                    return 'balance'
            amount_out = self.contract.functions.getAmountsOut(value, [(self.ETH, token_to_buy, False)]).call()[1]
            ...
            if tx_receipt.status == 1:
                ...
                if TgBot.TG_BOT_SEND is True:
                    TgBot.send_message_error(self, self.number, 'Buy token Velocore', self.address_wallet,
            ...
            self.log.info(f'[{self.number}] Buy {min_tok} USDC Velocore || https://explorer.zksync.io/tx/{hash_}\n')
            if TgBot.TG_BOT_SEND is True:
                TgBot.send_message_success(self, self.number, f'Buy {min_tok} USDC Velocore', self.address_wallet,
    ...
    def sold_token(self, token_to_sold, retry=0):
        self.log.info('Sold token Velocore')
        try:
            ...
            if token_balance == 0:
                ...
                return
            amount_out = self.contract.functions.getAmountsOut(token_balance, [(token_to_sold, self.ETH, False)]).call()[1]
            min_tokens = int(amount_out * (1 - (1 / 100)))
            ...
            if tx_receipt.status == 1:
                ...
                self.log.info(f'Transaction failed. Try again')
                if TgBot.TG_BOT_SEND is True:
                    TgBot.send_message_error(self, self.number, 'Sold token Velocore', self.address_wallet,
                                             'Transaction failed. Try again')
                time.sleep(60)
                retry += 1
                if retry > 5:
            ...
            hash_ = str(tx_hash.hex())
            self.log.info(f'[{self.number}] Sold {min_tok} USDC Velocore || https://explorer.zksync.io/tx/{hash_}\n')
            if TgBot.TG_BOT_SEND is True:
                TgBot.send_message_success(self, self.number, f'Sold {min_tok} USDC Velocore', self.address_wallet,
                                           f'https://explorer.zksync.io/tx/{hash_}')
        except TransactionNotFound:
            self.log.info('Transaction not found for a while. Try again')
            if TgBot.TG_BOT_SEND is True:
